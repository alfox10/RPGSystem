...
def retrieve_player_stat(p_id):
    ...
    if conn is None:
        create_connection()
    cur = conn.cursor()
    sql = """ SELECT hp, max_hp, omens, max_omens, name, pg_class, class_type, level, exp FROM player_stats WHERE player_id=? """
    return cur.execute(sql, p_id).fetchall()
...
def retrieve_attributes(p_id,table):
    global conn
    if conn is None:
        create_connection()
    cur = conn.cursor()
    sql = """ SELECT agility,presence,strenght, toughness FROM ? WHERE player_id=? """
    return cur.execute(sql, table, p_id).fetchall()
